...
def rot6d_to_rotmat(x):
    ...
    a1 = x[:, :, 0] 
    a2 = x[:, :, 1]
    b1 = F.normalize(a1) # 회전축 단위 벡터 #
    b2 = F.normalize(a2 - torch.einsum('bi,bi->b', b1, a2).unsqueeze(-1) * b1) # 다른 coordinate의 좌표 #
    b3 = torch.cross(b1, b2) # 회전 축에 대한 외적 행렬 #
...
def rotation_matrix_to_angle_axis(rotation_matrix):
    ...
    return aa # [N, 3] -> 최종적으로 3개의 값으로 구성된 vector로 angle-axis를 나타냄!! #


def quaternion_to_angle_axis(quaternion: torch.Tensor) -> torch.Tensor:
    """ 회전 축, 회전 각으로 나타내는 것이 angle-axis of rotation이 됨
    회전 축에 대해서 theta만큼 회전할때의 rotation matrix를 quaternion으로 나타내면
    qvq* = Mv이고, M이 그 회전 행렬이 될 것이다.
    This function is borrowed from https://github.com/kornia/kornia
    ...
    # q = cos(theta/2) + 회전축단위벡터 x sin(theta/2)
    sin_theta: torch.Tensor = torch.sqrt(sin_squared_theta) ## ||q|| ##
    cos_theta: torch.Tensor = quaternion[..., 0] ## q0 ##
    # theta = 2 x acos(a) #
...
def rotation_matrix_to_quaternion(rotation_matrix, eps=1e-6):
    """
    qvq* = v' (회전축에 대해서 theta만큼 v를 회전한 결과)
    q = q0 + i x q1 + j x q2 + k x q3
    v = i x v1 + j x v2 + k x v3
    q* = q0 - i x q1 - j x q2 - k x q3
    = Mv
    This function is borrowed from https://github.com/kornia/kornia
