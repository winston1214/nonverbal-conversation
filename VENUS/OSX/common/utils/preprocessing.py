...
def process_bbox(bbox, img_width, img_height):
    ...
    '''이 부분은 실제 rendering하는 것과는 다른 문제로, 그냥 detection bounding box의 크기를 1.25배로 키우면  '''
    bbox[2] = w * 1.25 # 0.75 # w * 1.25
    bbox[3] = h * 1.25 # 0.75 # h * 1.25
    bbox[0] = c_x - bbox[2] / 2.
    bbox[1] = c_y - bbox[3] / 2.
...
def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape):
    ...
    bb_c_x = float(bbox[0] + 0.5 * bbox[2]) ## bounding box center (X) (top-left x coord + bbox_width // 2)
    bb_c_y = float(bbox[1] + 0.5 * bbox[3]) ## bounding box center (Y) (top-left y coord + bbox_height // 2)
    ...
    trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot) # rotation이나 scaling도 할 수도 있기 때문에 affine transformation을 적용하는 관점에서 고려 #
