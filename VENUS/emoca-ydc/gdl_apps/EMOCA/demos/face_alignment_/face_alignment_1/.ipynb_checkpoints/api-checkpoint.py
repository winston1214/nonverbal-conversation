...
class FaceAlignment_:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        # face_detector_module = __import__('.face_alignment_1.detection.' + face_detector,
        #                                   globals(), locals(), [face_detector], 0) 
        face_detector_module = sfd

        # print(f'face_detector_module: {face_detector_module}')
        ...
        # print(f'face_detector device: {self.face_detector.device}')
    ...
    def get_landmarks_from_image(self, image_or_path, detected_faces=None, return_bboxes=False,
        ...
        """
        image = get_image(image_or_path) # tensor on cpu
        ...
        for i, d in enumerate(detected_faces):
            # image size (1080, 1920, 3) (H,W,C)
            # detection output  [750,392,1140,906,1]
            # maby [x_min, y_min, x_max, y_max, score]?
            center = torch.tensor( # [x_center, y_center]
                [d[2] - (d[2] - d[0]) / 2.0, # 1140 - (1140-750)/2 = 945
                 d[3] - (d[3] - d[1]) / 2.0]) # 906 - (906-392)/2 = 649
            # shift center down by 12% of the height.... why?
            center[1] = center[1] - (d[3] - d[1]) * 0.12 
            # scale is bbox height + bbox width / 195
            # ((1140-750) + (906-392)) / 195 = 4.5
