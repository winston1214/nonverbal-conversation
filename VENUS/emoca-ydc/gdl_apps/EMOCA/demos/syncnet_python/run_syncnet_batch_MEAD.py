...
if __name__=='__main__' :
    ...
    actors = os.listdir(args.audio_path)
    print(f'actors : {actors}')
    for i, actor in enumerate(actors) :
        audio_paths = glob.glob(f'{args.audio_path}/{actor}/*.wav')
        for audio_path in tqdm(audio_paths, desc=f'{actor} {i}/{len(actors)}') :
            file_name = os.path.basename(audio_path).split('.')[0]
            # frame_paths = sorted(glob.glob(f'{args.frame_path}/{emotion_name}/{file_name}_idM003_/*.png'))
            # frame_paths = sorted(glob.glob(f'{args.frame_path}/{actor}/{file_name}_idM003__0/*.png'))
            frame_paths = sorted(glob.glob(f'{args.frame_path}/{actor}/{file_name}/*.png'))
