...
def main(syncnet_model, face_model, image_paths, audio_path, args, transform, device) :
    ...
    #     # input_images = input_images[25:len(input_images)-5]
...
if __name__=='__main__' :
    ...
    if args.dataset == 'VOXCELEB': # 224X224
        ...
        if args.dataset == 'RAVDESS' :
            ...
            val_ids = ['wav_subsets_0', 'wav_subsets_1', 'wav_subsets_2']
    ...
    for val_id in val_ids :
        audio_paths = glob.glob(f'{args.audio_path}/{val_id}/*.wav')
        for audio_path in audio_paths :
            file_name = os.path.basename(audio_path).split('.')[0]
            # emotion = int(file_name.split('_')[1])
            # emotion = modify_DICT[emotion]
            # DEEPTalk
            frame_paths = sorted(glob.glob(f'{args.frame_path}/{val_id}/images/{file_name}_idM003__0/*.png'))
            # FaceFormer
            # frame_paths = sorted(glob.glob(f'{args.frame_path}/{val_id}/images//{val_id}/{file_name}_idM003_/*.png'))
            # else
            # frame_paths = sorted(glob.glob(f'{args.frame_path}/{val_id}/{file_name}/*.png'))
            if frame_paths == [] :
                continue
