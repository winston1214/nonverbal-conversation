...
3. thumbnail_filter.py
   ...
   ```
4. Audio file download -> segment_id_list.csv (segment_time = 600 sec)
   ```
   python video_audio_download.py --save_path $SAVE_PATH --mode wav --org_del 1
   ```
## (b) ASR Transcripts
5. Run whisper (STT) -> make filtering_file_list.csv
   ```
   python run_whisper.py --save_path $SAVE_PATH --hf_token $YOUR_HF_TOKEN
   ```
6. Cleaning folder to save disk memory -> make step1_segment_id_list.csv
   ```
   python cleaner.py --mode whisper --save_path $SAVE_PATH
   ```
7. Preprocessing for whisper results
   ...
   ```
8. Video download (Only video without audio)
   ```
   python video_audio_download.py --mode video --save_path $SAVE_PATH
   ```
9. Check video (Check if the video download was successful) -> make step2_2_segment_id_list.csv
   ```
   python checking.py --save_path $SAVE_PATH --mode check_fps 
   ```
## (c) Identifying Speaker
10. RUN ASD (Active Speaker Detection)
   ```
   python step2_main.py --save_path $SAVE_PATH --batch_size $YOLO_BATCH_SIZE --weight_path $ASD_WEIGHT_PATH
   ```
11. Remove video with fixed frames
   ```
   python cleaner.py --save_path $SAVE_PATH --mode video
   ```
12. Cropping speaker -> make segment.csv
    ```
    python crop_person_single.py --mode full --save_path $SAVE_PATH
    ```
## (d) Extracting Nonverbal-Cues
13. Extract mesh parameter (Body language)
   ```
   cd OSX/demo/ && python demo_batch.py
   ```
14. Extract mesh parameter (Facial Expression)
   ```bash
